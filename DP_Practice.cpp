...
void solve(int test) {//LIS = longest increasing Subsequence
    ...
    //# dp optimization korchi
    // segment tree use kore 
    // i number index a j index ar value a[i] seta rakhchi that means tar nicher gula count 
    // korlye hoy seta segment tree dia kore fela jay
    for (int i = 1; i <= n;i++){
        dp[i] = 1;
        if(a[i-1]!=1){
